 ...
 */
int 
zpool_release_one(zpool_handle_t *zhp,void *data)
{
	export_cbdata_t *cb = data;
...
zpool_do_release(int argc, char **argv )
{
	...
	if ((zhp = zpool_open_canfail(g_zfs, pool)) == NULL) {
		/*
		 * As a special case, check for use of '/' in the name, and
		 * direct the user to use 'zfs release' instead.
		 */
		if (strchr(pool, '/') != NULL)
			(void) fprintf(stderr, gettext("use 'zfs release' to "
			    "release a dataset\n"));
		return (1);
	}

	if (zpool_disable_datasets(zhp, force) != 0) {
		(void) fprintf(stderr, gettext("could not release '%s': "
		    "could not unmount datasets\n"), zpool_get_name(zhp));
		zpool_close(zhp);
		return (1);
	}
	...
	ret = zpool_sync_one(zhp,&force);
	//ret = for_each_pool(argc, argv, B_FALSE, NULL, zpool_sync_one, &force);
	ret = ret & zpool_release_one(zhp, history_str);	
	zpool_close(zhp);
